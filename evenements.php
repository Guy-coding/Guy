...
try {
    ...
    if (isset($_POST['delete_event'])) {
        ...
        $stmt->execute();
        echo "<script> alert('Evenement supprimé avec succès !'); window.location.href = 'formevent.php'</script>";
    }
  ...
  <div class="card h-100">
    ...
    <div class="card-body d-flex flex-column">
      ...
      <div class="card-footer d-flex justify-content-between">
            <p class="card-text">Date : <?php echo $article['date_deroulement']; ?></p>
          </div>
      <p class="card-text"><?php echo substr($article['description'], 0, 300) . '...'; ?></p>
      <div class="mt-auto d-flex justify-content-end">
        <form method="post" action="">
          <input type="hidden" name="event_id" value="<?php echo $article['id']; ?>">
          <button type="submit" name="delete_event" class="btn btn-primary me-2">Supprimer</button>
        </form>
        <!-- <a class="btn btn-primary" href="modification_blog.php">Modifier</a> -->
        <a href="modification_evenements.php?id=<?php echo $article['id']; ?>"" class="btn btn-primary me-2">
  Modifier
</a>
        <a class="btn btn-primary" href="detailleevenements.php?id=<?php echo $article['id']; ?>">Détails</a>
      </div>
    </div>
      </div>
    </div>
